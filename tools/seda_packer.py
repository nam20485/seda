...
def extract_payload():
    ...
    if 'POST_INSTALL' in globals():
        ...
        if cmd:
            try:
                # SECURITY: Commands are provided by archive creator (trusted source).
                # Review POST_INSTALL contents before running untrusted archives.
...
def encrypt_payload(payload_dict, password):
    ...
    for k, v in payload_dict.items():
        if isinstance(v, bytes):
            clean_payload[k] = "BASE64:" + v.decode(
                "utf-8"
            )  # Packer stored b64 as bytes in dict
        else:
            clean_payload[k] = v
...
def generate_seda(
    ...
    base_output = (
        output_filename
        if output_filename
        else os.path.basename(os.path.abspath(source_dir))
    )

    # Strip extensions (case-insensitive match)
    base_lower = base_output.lower()
    for ext in [
        ".smartpatch.seda",
        ".construct.seda",
        ".commit.seda",
        ...
        if base_lower.endswith(ext):
            base_output = base_output[: len(base_output) - len(ext)]
            break
    ...
    for root, dirs, files in os.walk(source_dir):
        ...
        for file in files:
            if any(file.endswith(ext) for ext in DEFAULT_IGNORE_EXTENSIONS):
                continue
            ...
            try:
                ...
                with open(full_path, "rb") as f:
                    ...
                    file_data[rel_path] = "BASE64:" + base64.b64encode(f.read()).decode(
                        "utf-8"
                    )
                print(f"   ➕ Added (binary): {rel_path}")
    ...
    try:
        with open(output_name, "w", encoding="utf-8") as out:
            ...
            if post_install:
                out.write(f"POST_INSTALL = {repr(post_install)}\n\n")
        ...
        print(
            f"❌ Error: Could not write archive '{output_name}': {e}", file=sys.stderr
        )
        sys.exit(1)
    ...
    try:
        ...
        print("   ⚠️ Warning: Could not set executable permissions")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vault", action="store_true", help="Generate Type 2 Encrypted Vault"
    )
    ...
    if args.post_install:
        if "win:" in args.post_install:
            ...
            post_map = {"universal": args.post_install}
