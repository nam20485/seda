...
- Post-install functionality (--post-install flag)
- Error handling
"""
...
class TestPackerType0Archives:
    ...
    def test_create_type0_archive(self, temp_dir, sample_project, packer_script):
        ...
        result = conftest.run_script(
            packer_script, [str(sample_project["root"]), str(output_file)]
        )
    ...
    def test_type0_archive_is_valid_python(
        self, temp_dir, sample_project, packer_script
    ):
        ...
        assert validation["is_valid_python"], (
            f"Archive is not valid Python: {validation.get('error')}"
        )
...
class TestPackerRecursiveSeda:
    ...
    def test_seda_files_ignored_by_default(self, temp_dir, packer_script):
        ...
        result = conftest.run_script(
            packer_script, [str(project_dir), str(output_file)]
        )
    ...
    def test_recursive_pack_seda_flag(self, temp_dir, packer_script):
        ...
        result = conftest.run_script(
            packer_script, [str(project_dir), str(output_file), "--recursive-pack-seda"]
        )
...
class TestPackerDefaultIgnores:
    ...
    def test_default_ignore_pycache(self, temp_dir, packer_script):
        ...
        assert result["success"]
        assert (
            "__pycache__" not in result["stdout"]
            or "__pycache__" not in output_file.read_text()
        )
...
class TestPackerErrorHandling:
    ...
    def test_nonexistent_directory_error(self, temp_dir, packer_script):
        ...
        result = conftest.run_script(
            packer_script, [str(nonexistent), str(output_file)]
        )
    ...
    def test_empty_directory(self, temp_dir, packer_script):
        ...
        result = conftest.run_script(packer_script, [str(empty_dir), str(output_file)])
...
class TestPostInstallExtensionMandating:
    """Tests for extension mandating with --post-install flag."""

    def test_post_install_produces_construct_extension(
        self, temp_dir, sample_project, packer_script
    ):
        """Test that --post-install flag produces .construct.seda extension (Type 1)."""
        output_base = temp_dir / "test_construct"
        ...
        # Should create test_construct.construct.seda
        expected_file = temp_dir / "test_construct.construct.seda"
        assert expected_file.exists(), f"Expected {expected_file} to exist"

        content = expected_file.read_text(encoding="utf-8")
        assert "POST_INSTALL" in content

    def test_post_install_with_docstring_produces_smartpatch_extension(
        self, temp_dir, sample_project, packer_script
    ):
        """Test that --post-install + --docstring produces .smartpatch.seda extension (Type 1+5)."""
        output_base = temp_dir / "test_smartpatch"
        docstring = "Feature: Add post-install hook"
        ...
        # Should create test_smartpatch.smartpatch.seda
        expected_file = temp_dir / "test_smartpatch.smartpatch.seda"
        assert expected_file.exists(), f"Expected {expected_file} to exist"
    ...
    def test_docstring_only_produces_commit_extension(
        self, temp_dir, sample_project, packer_script
    ):
        """Test that --docstring alone produces .commit.seda extension (Type 5)."""
        output_base = temp_dir / "test_commit"
        docstring = "Commit message only"

        result = conftest.run_script(
            packer_script,
            [str(sample_project["root"]), str(output_base), "--docstring", docstring],
        )

        assert result["success"]

        # Should create test_commit.commit.seda
        expected_file = temp_dir / "test_commit.commit.seda"
        assert expected_file.exists()

        content = expected_file.read_text(encoding="utf-8")
        # Check that POST_INSTALL variable is not assigned (extraction logic has the name but no assignment)
        assert "POST_INSTALL = {" not in content and "POST_INSTALL={" not in content


class TestPostInstallCommandMapping:
    """Tests for post-install command parsing and mapping."""

    def test_universal_command_mapping(self, temp_dir, sample_project, packer_script):
        """Test parsing of --post-install 'npm install' produces universal mapping."""
        output_file = temp_dir / "test.seda"
        ...
        actual_file = temp_dir / "test.construct.seda"
        content = actual_file.read_text(encoding="utf-8")

        # Check that POST_INSTALL has universal command
        assert "POST_INSTALL" in content
        assert (
            "'universal': 'npm install'" in content
            or '"universal": "npm install"' in content
        )

    def test_platform_specific_command_mapping(
        self, temp_dir, sample_project, packer_script
    ):
        """Test parsing of --post-install 'win:npm install,unix:make install' produces platform mappings."""
        output_file = temp_dir / "test.seda"
        ...
        result = conftest.run_script(
            ...
            [
                ...
                str(output_file),
                "--post-install",
                "win:npm install,unix:make install",
            ],
        )
        ...
        # Check that POST_INSTALL has platform-specific commands
        assert "POST_INSTALL" in content
        assert "'nt': 'npm install'" in content or '"nt": "npm install"' in content
        assert (
            "'posix': 'make install'" in content or '"posix": "make install"' in content
        )

    def test_windows_only_command_mapping(
        self, temp_dir, sample_project, packer_script
    ):
        """Test parsing of --post-install 'win:echo hello' produces Windows-only mapping."""
        output_file = temp_dir / "test.seda"
        ...
        assert "POST_INSTALL" in content
        assert "'nt': 'echo hello'" in content or '"nt": "echo hello"' in content

    def test_unix_only_command_mapping(self, temp_dir, sample_project, packer_script):
        """Test parsing of --post-install 'unix:make build' without win: prefix produces universal mapping.

        Note: The packer only parses platform-specific commands when 'win:' prefix is present.
        Without 'win:', the entire string is treated as universal.
        """
        output_file = temp_dir / "test.seda"
        ...
        assert "POST_INSTALL" in content
        # Without 'win:' prefix, the string is treated as universal (literal value)
        assert (
            "'universal': 'unix:make build'" in content
            or '"universal": "unix:make build"' in content
        )


class TestPostInstallArchiveContent:
    """Tests for POST_INSTALL variable in generated archives."""

    def test_post_install_variable_in_archive(
        self, temp_dir, sample_project, packer_script
    ):
        """Test that generated archive contains POST_INSTALL dictionary."""
        output_file = temp_dir / "test.seda"
        ...
        # Check POST_INSTALL variable format
        assert "POST_INSTALL = {" in content or "POST_INSTALL={" in content
        assert "pip install -r requirements.txt" in content

    def test_post_install_with_complex_command(
        self, temp_dir, sample_project, packer_script
    ):
        """Test POST_INSTALL with complex multi-part command."""
        output_file = temp_dir / "test.seda"
        complex_cmd = "npm install && npm run build"
        ...
        assert complex_cmd in content

    def test_archive_without_post_install(
        self, temp_dir, sample_project, packer_script
    ):
        """Test that archives without --post-install don't have POST_INSTALL variable."""
        output_file = temp_dir / "test.seda"
        ...
        actual_file = temp_dir / "test.seda"
        content = actual_file.read_text(encoding="utf-8")

        # Should NOT have POST_INSTALL variable assignment (extraction logic has the name but no assignment)
        assert "POST_INSTALL = {" not in content and "POST_INSTALL={" not in content

    def test_post_install_archive_is_valid_python(
        self, temp_dir, sample_project, packer_script
    ):
        """Test that generated archive with POST_INSTALL is valid Python."""
        output_file = temp_dir / "test.seda"
        ...
        actual_file = temp_dir / "test.construct.seda"
        validation = conftest.validate_seda_archive(actual_file)
